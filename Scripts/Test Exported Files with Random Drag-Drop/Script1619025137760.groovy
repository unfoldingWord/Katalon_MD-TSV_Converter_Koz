...
moves = 15 // Number of random row drags before export
...
testFiles.each{ def book ->
    ...
    dragRows(moves, rowCount)

    for (def box : checkboxes) {
    ...
    for (def text : sRowText) {
        ...
        if (((((sr + oq) + oc) + glq) + on) != '') {
            (tRowText[row]) = ((((((((((((ref + tab) + id) + tab) + sr) + tab) + oq) + tab) + oc) + tab) + glq) + tab) + 
            on)
        } else {
            (tRowText[row]) = null
        }
	...
	WebUI.callTestCase(findTestCase('Test TSV in tC Create'), [('myFile') : myFile, ('bookNumber') : bookNumbers[n]])
...
def getTableValues(def xPath, def type, def refType) {
    if (refType == 'Book') {
        checkboxColumn = 2
    } else {
        checkboxColumn = 1
    ...
    rows.each({ 
            ...
            List columns = rows.get(row).findElements(By.tagName('td'))
...
def duration(def start, def end) {
    seconds = ((end.getTime() - start.getTime()) / 1000)

    return ((seconds) as int) + ' seconds'
}

def timeForCheck(def object) {
    maxWait = 600
